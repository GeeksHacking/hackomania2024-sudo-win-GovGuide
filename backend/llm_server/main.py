...
from .prompts import (
	EXPLAIN_SCHEME_TO_USER,
	GENERATE_DRAFT_SCRIPT,
	ENHANCE_SCRIPT_CHECK,
	ENHANCE_SCRIPT_WRITE,
	FORMAT_SCRIPT,
	GET_RESOURCES
)
...
def split_text(text, max_=15, min_=5):
	...
	for i in range(0, len(split_text), max_):
		...
		if len(split_text) - upper_lim < min_:
			new_content.append(" ".join(split_text[i:]) + " ")
			break
		else:
			new_content.append(" ".join(split_text[i:upper_lim]) + " ")
...
class ScriptGenerator:
	def __init__(self):
		...
		self.chat4 = ChatOpenAI(model='gpt-4-0125-preview', api_key=env["OPENAI_API_KEY"])
	...
	def format_script(self, script: str) -> str:
		json_content = self.json_chat.invoke([HumanMessage(content=FORMAT_SCRIPT.format(script=script))]).content
	...
	def enhance_script(self, user_data: UserData, explanations: List[str], draft_script: str) -> str:
			...
			[
				HumanMessage(
					content=ENHANCE_SCRIPT_CHECK.format(
						info=user_data, explanations=explanations, draft_script=draft_script
					)
				)
			]
		).content
		...
		out = self.chat.invoke(
			[HumanMessage(content=ENHANCE_SCRIPT_WRITE.format(draft_script=draft_script, improvements=improvements))]
		).content
	...
	def __generate_draft_script(self, user_data: UserData, explanations: List[str]) -> str:
		...
		return self.chat.invoke(
			[HumanMessage(content=GENERATE_DRAFT_SCRIPT.format(info=user_data, explanations=explanations))]
		).content
	...
	def __explain_scheme_to_user(self, user_data: UserData, doc: str) -> str:
		...
		out = self.chat.invoke(
			[HumanMessage(content=EXPLAIN_SCHEME_TO_USER.format(info=user_data, scheme=doc))]
		).content
		print("explain:", out)
		return out
